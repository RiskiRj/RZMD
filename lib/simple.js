...
exports.makeWASocket = (...args) => {
     ...
     */
     conn.getBuffer = getBuffer = async(url) => {
        const res = await fetch(url, { headers: { 'User-Agent': 'okhttp/4.5.0' }, agent: httpsAgent, method: 'GET' })
    ...
    conn.fetchJson = fetchJson = (url, options) => new Promise(async(resolve, reject) => {
        fetch(url, options)
    ...
    conn.getFile = async (PATH, returnAsFilename) => {
        let res, filename
        let data = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH, {agent: httpsAgent})).buffer() : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
    ...
    */
    conn.sendFile = async (jid, path, fileName = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        .then(() => {
            ...
            fs.existsSync(type.filename) && fs.unlinkSync(type.filename);
        })
    }
...
END:VCARD
    ...
    // const { generateWAMessageFromContent, proto } = require('@danielteodoro/baileys-md')
